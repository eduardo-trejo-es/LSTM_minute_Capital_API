...
class Model_Trainer:
  ...
  def to_tain(self,modelPath,DatasetPath,ThepercentageTrainingData):
    ...
    DS_raw_Time_scaled=DS_raw_scaled[:,[5]]
    ...
    for i in range(n_past, len(DS_raw_scaled) - n_future +1):
      ...
      DS_finished_Time_Y.append(DS_raw_Time_scaled[i:i+1, 0:DS_raw_scaled.shape[1]])
    ...
    DS_finished_Close_Y, DS_finished_Volume_Y, DS_finished_Time_Y = np.array(DS_finished_Close_Y), np.array(DS_finished_Volume_Y), np.array(DS_finished_Time_Y)
    ...
    trainY_Time,testingY_Time = Split3DimData(DS_finished_Time_Y,percentageTrainingData)
    ...
    model.fit(x=trainX,y=y_data, epochs=15, batch_size=15, validation_data=(testingX,testing_y_data),callbacks=[early_stop])
